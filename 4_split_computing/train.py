...
parser.add_argument('-c', '--checkpoint', default='checkpoints/compression/', type=str, metavar='PATH',
...
def train(train_loader, train_loader_len, model, criterion, optimizer, adjuster, epoch):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        new_ratio = np.random.uniform(0.1, 0.5)
        model.module.encoder.bottleneck_ratio = new_ratio
        model.module.decoder.bottleneck_ratio = new_ratio
        ...
        bar.next()
    bar.finish()
    model.module.encoder.bottleneck_ratio = 0.5
    model.module.decoder.bottleneck_ratio = 0.5
